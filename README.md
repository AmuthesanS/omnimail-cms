...
Needs **JDK 17+**, **Maven**, **Docker**, and **Docker Compose** (`docker compose` plugin or `docker-compose` CLI). On Ubuntu:

```bash
sudo apt install docker.io docker-compose-v2
```

Add your user to the `docker` group if needed: `sudo usermod -aG docker $USER` (log out and back in).
...
Blob bytes are stored locally under `OMNIMAIL_DATA_DIR` (default `./data/blobs`) or in GCS when `omnimail.storage.type=gcs`.
